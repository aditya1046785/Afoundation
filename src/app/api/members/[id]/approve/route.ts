...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    ...
    if (action === "approve") {
        ...
        await sendMemberApprovedEmail(member.user.email, member.user.name, member.memberId);
        ...
        await sendMemberRejectedEmail(member.user.email, member.user.name, reason || "Your application did not meet our current requirements.");
